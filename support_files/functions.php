...
if (!function_exists('get_in')){
	...
	function get_in($username){
		include('dbinfo.inc');
		$get_query = $con->query("SELECT * FROM access WHERE username = '$username'")->fetch_assoc();
		$key = $get_query["key"];
		$phrase = $get_query["phrase"];
		$data = base64_decode($phrase);
		$iv_size = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_CBC);
		$iv = substr($data, 0, $iv_size);
		return rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_128,hash('sha256', $key, true), substr($data, $iv_size), MCRYPT_MODE_CBC, $iv),"\0");
	}
}
